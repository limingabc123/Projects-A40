...
from eval.long_recon.base import BaseStereoViewDataset
import eval.long_recon.dataset_utils.cropping as cropping
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            ...
            if self.num_frames > 0:
                num_files = min(self.num_frames, num_files)
...
class DTU(BaseStereoViewDataset):
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if not self.full_video:
            ...
            num_files = len(img_idxs)
            if self.num_frames > 0:
                num_files = min(self.num_frames, num_files)
            img_idxs = img_idxs[:num_files]
            img_idxs = img_idxs[:: self.kf_every]
...
class NRGBD(BaseStereoViewDataset):
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            ...
            if self.num_frames > 0:
                num_files = min(self.num_frames, num_files)
            start_frame = min(self.start_frame, num_files - 1)
            img_idxs = [f"{i}" for i in range(start_frame, num_files)]
        ...
        while len(imgs_idxs) > 0:
            ...
            views.append(
                dict(
                    img=rgb_image,
                    depthmap=depthmap,
                    camera_pose=camera_pose, # camera-to-world
                    camera_intrinsics=intrinsics,
                    dataset="nrgbd",
                    label=osp.join(scene_id, im_idx),
                    instance=impath,
