...
def eval_metrics(pred_traj, gt_traj=None, seq="", filename="", sample_stride=1):
    ...
    def _stats(result):
        arr = result.np_arrays["error_array"]
        return {
            "max":    float(arr.max()),
            "min":    float(arr.min()),
            "mean":   float(arr.mean()),
            "median": float(np.median(arr)),
            "rmse":   float(result.stats["rmse"]),
            "sse":    float(result.stats["sse"]),
            "std":    float(result.stats["std"]),
        }

    metrics_dict = {
        "seq":       seq,
        "ate":       _stats(ate_result),
        "rpe_rot":   _stats(rpe_rots_result),
        "rpe_trans": _stats(rpe_transs_result),
    }
